...
fe_t <- fs_t %>% 
  mutate(across(c("area","population":"avg_age", "city_population", "population_density"),~.x**2,.names = "{.col}_2")) %>% 
  mutate(across(c("area","population":"avg_age", "city_population", "population_density"),~.x**3,.names = "{.col}_3"))
```
...
ptp_s <- post_test_performance(test_res_s, test_res_p$n_schools)
ptp_s
```
...
Poisson model seems to have better performance on test set.
