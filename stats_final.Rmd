...
```{r}
lm_model_pharmacy <- lm(n_pharmacies ~ ., data = fe_df)
summary(lm_model_pharmacy)
...
lm_fit_pharmacy
getTrainPerf(lm_fit_pharmacy) #Average performance across all models

#Importance variables
varImp(lm_fit_pharmacy)
ggplot(varImp(lm_fit_pharmacy))

#Predict on testData
predict_pharmacy_lm <- predict(lm_fit_pharmacy, testData)
predict_pharmacy_lm

test_phar <- testData %>% mutate(
  pred = predict_pharmacy_lm
)
test_phar %>% select(n_pharmacies, pred)

#Performance on the test
mean(lm_fit_pharmacy$resample$MAE) #train data MAE

postResample(
  pred = test_phar$pred,
  obs = test_phar$n_pharmacies
)
```
