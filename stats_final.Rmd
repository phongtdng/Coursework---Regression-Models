...
selected_features <- df %>% 
  select(-c(index, province_code, pcg_foreigners, geometry))
```
...
```{r}
toTrain <- createDataPartition(df$n_pharmacies, p = .80, list = FALSE)
...
```{r}
error_summary(train_b$n_pharmacies, mean(train_b$n_pharmacies))
```

### School

```{r}
error_summary(train_b$n_schools, mean(train_b$n_schools))
```

### Transport POS

```{r}
error_summary(train_b$n_transport_salespoints, mean(train_b$n_transport_salespoints))
```
