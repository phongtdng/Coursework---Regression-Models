...
It is noted that the performance of the Logistic Regression model is moderate( Accuracy = \`r getTrainPerf(logReg_final)\$TrainAccuracy\` ) and there might be more models that could perform better.

Using this model, we made final predictions on the original data set to obtain an estimate of which censuses are in need of more facilities. Below is a map showing censuses that might be lacking facilities.
